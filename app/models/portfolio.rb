class Portfolio < ApplicationRecord
  ...
  def money_market
    mm = self.stocks.where(symbol: 'SWVXX').or(self.stocks.where(symbol: 'SNVXX')).or(self.stocks.where(symbol: 'SNAXX')).sum(0) { |m| m.quantity }
    return mm
  end
  ...
  def self.table_data(group_id, sort_by)
    ...
    all.each { |p| totals[p.id] = [ (p.cash + p.total_stocks_value + p.total_options_value).to_f, p.total_stocks_change_value, p.cash ] }
